...
@SessionAttributes("publisherEditBean")
public class EditPublisher {
	
	private static Logger logger = Logger.getLogger(EditPublisher.class.getName());
	...
	public void setupNewForm(@RequestParam(value="publisherName", required=true) String name, Model modell) {
		logger.info(new StringBuffer("EditPublisher: name:").append(name).toString());
		
		PublisherEditBean publisherEditBean = teklibService.getPublisherByName(name);
		modell.addAttribute(publisherEditBean);
	}
	
	@RequestMapping(method = RequestMethod.POST)
	public String processSubmit(@ModelAttribute PublisherEditBean publisherEditBean, BindingResult result, SessionStatus status, String formAction) {
		logger.info(new StringBuffer("process submit:").append(publisherEditBean)
		...
		if(formAction != null && formAction.equals(FORM_ACTION_SUBMIT)) {
			bookValidator.validate(publisherEditBean, result);
			if (result.hasErrors()) {
				return "editPublisher";
			} else {
				this.teklibService.storePublisher(publisherEditBean);
				status.setComplete();
				return REDIRECT_HOME;
			}
		} else if(formAction != null && formAction.equals(FORM_ACTION_DELETE)) {
			this.teklibService.deletePublisher(publisherEditBean);
			status.setComplete();
			return REDIRECT_HOME;
